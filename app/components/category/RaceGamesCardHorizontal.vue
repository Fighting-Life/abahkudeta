...
const queryProvider = computed(() => route.query.p as string | undefined);
const allGames = computed(() => RaceMarblexGames);
...
const allCategoryGames = computed(() => {
	if (searchTerms.value) {
		return allGames.value.filter((game) =>
			game.name.toLowerCase().includes((searchTerms.value || "").toLowerCase()),
		);
	} else {
	...
	<div class="w-full space-y-6">
		...
		<div class="flex items-center gap-3">
			...
			<h2 class="text-xl font-bold text-white uppercase sm:text-2xl">
				{{ queryProvider?.replaceAll("-", " ") || "All Games" }}
			</h2>
			<ClientOnly>
