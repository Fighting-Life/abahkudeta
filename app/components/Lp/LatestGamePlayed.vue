...
const { $gameHistoryState } = useNuxtApp();
const { launchGameHistory } = useGameLauncher();
...
const totalPages = computed(() => {
	return Math.ceil($gameHistoryState.value.length / itemsPerPage.value);
});
...
const paginatedGames = computed(() => {
	...
	for (let i = 0; i < totalPages.value; i++) {
		...
		pages.push($gameHistoryState.value.slice(start, end));
	}
	return pages;
});
...
// Get image
function getImageGame(game?: GameHistory) {
	if (!game || !game.game_link || !game.game_image) {
		return "/images/no-image.webp";
	}

	return `${baseImgUrl.value(game.game_link)}${game.game_image}`;
}
